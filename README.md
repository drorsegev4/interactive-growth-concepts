...
- **Config-driven copy and interpolation.** Every user-facing string lives under `ui` in `config.json` — nothing is hardcoded in the HTML or JS. Where copy needs a computed value, `core.js` exports a tiny `interpolate(template, vars)` helper that replaces `{token}` placeholders. Available tokens: `{team}`, `{maxMultiplier}`, `{multiplier}`, `{odds}`, `{boostedOdds}`. `sports.headlines.A.headline` uses `{maxMultiplier}` specifically so the copy can never drift out of sync with the zone ladder — change the zones, the headline updates itself.
